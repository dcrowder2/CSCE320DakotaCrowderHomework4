...
void setTable(int forksCount)//done
{
...
void *philosopher(void *params)//todo
{
  int succesfulEatCount = 0, eatTries = 0;
  signal(SIGABRT, threadSignalHandler);
  params_t myInfo = *(params_t *)params;
  while(1)
  {
    printf(myInfo.name);
    printf(": I have tried to eat %u times, and ate %u times\n", eatTries, succesfulEatCount);
    think(myInfo.name);
    printf(myInfo.name);
    printf(": I am hungry\n");
    eatTries++;
